...
class LocalExpenseParser {
    func parse(_ text: String) -> ParseResult {
        ...
        let confidence: Float = (amount != nil ? 0.4 : 0.0) + (category != nil ? 0.3 : 0.0) + 0.2
    ...
    private func extractCategory(from text: String) -> String? {
        let categories = ["咖啡": "餐饮", "午餐": "餐饮", "打车": "交通"]
    ...
    private func extractNote(from text: String) -> String? {
        return text.count > 10 ? String(text.prefix(20)) : text
    }
}
...
// 执行测试
print("=== HybridParsingService调用链测试 ===")
print("测试时间: \(Date())")
print()
...
print("2. 测试HybridParsingService调用链:")
let hybridService = TestHybridParsingService()

Task {
    let result = await hybridService.parseExpenseText("我今天买咖啡花了25元")
    print("\n=== 最终结果 ===")
